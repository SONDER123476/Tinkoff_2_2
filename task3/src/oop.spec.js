...
describe('ООП', () => {
    describe('#Point', () => {
        ...
        it('Точка создается в центре, ищем расстояние до центра', () => {
            const point = new core.Point();
            assert.strictEqual(point.distanceToCenter(), 0);
        });
        it('Точка создается не в центре, ищем расстояние до центра', () => {
            const point = new core.Point(2,0);
            assert.strictEqual(point.distanceToCenter(), 2);
        });
    });
    ...
    describe('#Queue', () => {
        ...
        it('метод front возвращает первый элемент', () => {
            const queue = new core.Queue([1, -2, 3, 5]);
        ...
        it('метод back возвращает последний элемент', () => {
            const queue = new core.Queue([1, -2, 3, 5]);
            const backElement = queue.back();
            assert.strictEqual(backElement, 5);
            assert.strictEqual(queue.size, 4);
            assert.deepEqual(queue.array, [1, -2, 3, 5]);
        });

        it('метод getAtIndex возвращает элемент по указанному индексу', () => {
            const queue = new core.Queue([1, -2, 3, 5]);
            const elementAtIndex2 = queue.getAtIndex(2);
            assert.strictEqual(elementAtIndex2, 3);
            assert.strictEqual(queue.size, 4);
            assert.deepEqual(queue.array, [1, -2, 3, 5]);
        });
    });
